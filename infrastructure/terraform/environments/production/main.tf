data "terraform_remote_state" "bootstrap" {
  backend = "s3"
  config = {
    bucket = "${replace(var.domain_name, ".", "-")}-s3-terraform-state" # Ensure the bucket name is valid
    key    = "${replace(var.domain_name, ".", "-")}/bootstrap.tfstate"
...
module "static-website" {
  ...
  s3_bucket_name     = "${replace(var.domain_name, ".", "-")}-s3-${var.environment}-website-assets"
