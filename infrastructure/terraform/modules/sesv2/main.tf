...
resource "aws_route53_record" "dkim" {
  ...
  name    = "${aws_sesv2_email_identity.main.dkim_signing_attributes[0].tokens[count.index]}._domainkey.${var.root_domain_name}"
  type    = "CNAME"
  ttl     = 600
...
resource "aws_sesv2_email_identity_mail_from_attributes" "main" {
  ...
  mail_from_domain = "mail.${var.root_domain_name}"
